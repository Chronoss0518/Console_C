...
int main()
{
    ...
    ※C言語では数値としての0以外は`true`、0はfalseとして扱われます。
    ...
    */
    ...
    if (0) return 0;//{}を除いて次の;までをif文として利用することが可能
    ...
    1つのswitch文内ではdefaultは1度しか記述できません。
    ...
    */
    ...
    while文は条件が正しい間のみ中の処理を行います。

    do whileとして、必ず1度は中の処理を行う繰り返し文も利用することができます。
